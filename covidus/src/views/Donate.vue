        ...
        <v-container v-if="success" ass="fill-height" style="width: 500px">
            <v-layout class="align-center justify-center">
                <v-flex class="text-xs-center">
                    <v-card flat line>
                        <v-alert
                          :value="success" color="green" dark dense border="top" icon="mdi-alert-circle" transition="scale-transition">
                         {{this.successMSG}}
                        </v-alert>
                    </v-card>
                </v-flex>
...
export default {
    ...
    data() {
        return {
            ...
            quantity: '',
            contact: '',
            typesEN: [ 'mask', 'protective glasses', 'gloves', 'face shields' ],
            typesFR: [ 'masque', 'lunettes de protection', 'gants', 'visière de sécurité' ],
            typesES: [ 'máscara', 'gafas de protección', 'guantes', 'caretas' ],
            typesRU: [ 'маска', 'защитные очки', 'перчатки', 'щитки для лица' ],
            error: false,
            errorMSG: '',
            success: false,
            successMSG: '',
    ...
    methods: {
        submit() {
            if (!this.location || !this.type || !this.quantity || !this.contact) {
                ...
                axios.post('http://localhost:6969/api/saveMaterials', {
